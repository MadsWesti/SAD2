...
These two algorithms are both extensions to MinHashing. In the worst case b-Bit has a 21-fold improvement in space complexity in comparison to how the data is saved in LSH. As long as the $k$ value is large enough, b-Bit will have about the same correctness as LSH. \todo{k-value - er det rigtigt nok}
...
\section{Conclusion}
By using MinHashing and Locality Sensitive Hashing, we got a pretty good approximation of similar pairs in the \emph{IMDB} dataset within a time span that was acceptable. It seems like b-bit could have made it faster, or at least more space efficient, but a succesful implementation was not reachable before the project handin. \\

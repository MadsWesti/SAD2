...
In the following table \ref{tab:lsh-correctness} the top 10 similar pairs from the LSH implementation can be seen together with their actual Jaccard similarities. This was run on dummy data. The worst difference is pair number 8, with a difference of $0.7 - 0.5 = 0.2$.\\

Both the actual pairs with similarity $1.0$ has been found. There were only 18 pairs with an actual similarity of $0.667$. There were over 100 pairs with similarity $0.5$. The similarities that LSH outputs is really similarities calculated by the MinHashing algorithm, including false positives and excluding false negatives from the LSH implementation. \\

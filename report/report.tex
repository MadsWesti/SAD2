...
For each algorithm we will explain how it works and then go on to explain how we implemented it. We then explain the time and space complexity and the tradeoffs in accuracy. We finish each section with a section about the correctness of the implementation. \\
...
As in the naive solution, we still have to compare all pairs of actors, but the number values needed to compared for each pair of actors is reduced from $m$ to $k$, resulting in a new running time $O(n^2 \cdot k)$ compared to previous $O(n^2 \cdot m)$, see figure ~\ref{fig:char_matrix} versus ~\ref{fig:signature_matrix}.
...
The MinHashing implementation is much faster than the naive, but we sacrifice some correctness. With the number of hash functions $k$, we can tune the correctness up or down with $k$ (see figure \ref{tab:minhash-correctness}), which will also inflict the running time, as seen in figure \ref{fig:minhashing_at}. \\
...
\subsection{Correctness}
In the following table \ref{tab:lsh-correctness} the top 10 similar pairs from the LSH implementation can be seen together with their actual Jaccard similarities. This was run on dummy data with $k = 1000$, and $b = 100$. This gives us a threshold of $t = (1/b)^{1/r} = 0.63$, which indicates that if a pair has similarity 0.63, there is 50\% chance for it to get selected.  \\
...
If a set has the same $k$ and is saved in 1 bit it will have an increased variance compared to being saved in 64 bit. To counteract this we can increase $k$. Jeg bliver lige nødt til at få det her ud på dansk. Det er klart at hvis man gemmer i 1 bit istedet for 64 bit mister man noget præcision. Ved at lave nogle flere permutations kan man opnå samme præsision. Hvor meget skal k stige for at opnå samme præcision? Ikke ret meget har jeg læst mig til. Man sparer simpelthen så meget plads at man kan øge præcisionen og stadig spare plads. Det er en winwin situation!

\subsection{Running Time}
Our implementation of the b-Bit algorithm does not succesfully run on the imdb dataset, only on our dummy data. Part of the reasom for this is that we have not been able to implement a data type with a small enough overhead. So instead of our data actually taking up less space, it takes up the same space, and furthermore additional processing is done to it. \\
...
These two algorithms are both extensions to MinHashing. In the worst case b-Bit has a 21-fold improvement in space complexity in comparison to how the data is saved in LSH. As long as the $k$ value is large enough, b-Bit will have about the same correctness as LSH.
